  ...
  <form id="formInput" class="row g-3">
    <div class="col-md-7">
      ...
      <div class="image-container" style="margin-bottom: 20px;">
        <img id="image" src="{{ asset(env('GLOBAL_PLUGIN_PATH').'/template/images/clients/'.$clients->data->image) }}" alt="{{ $clients->data->image_ori }}">
      </div>
      <input type="file" name="file" class="form-control" id="uploadImage" accept="image/*">
    </div>
    <div class="col-md-12">
      <label for="formFile" class="form-label">Title</label>
      <input type="text" name="title" class="form-control" value="{{$clients->data->title}}" required>
    </div>
    <div class="text-center">
      <input type="hidden" name="file_before" value="{{$clients->data->image}}">
  ...
  $(document).ready(function() {
    ...
    $('#formInput').on("submit", function(event) {
      ...
      if ($('#formInput').valid()) { // Check if form is valid
        ...
        $.ajax({
          type: "POST",
          url: "{{ URL::asset(env('APP_URL').'/admin-page/clients/do-add') }}",
          data: data,
          processData: false,
          contentType: false,
          dataType: "json",
