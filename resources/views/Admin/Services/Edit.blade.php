  ...
  <form id="formInput" class="row g-3">
    <div class="col-md-7">
      ...
      <div class="image-container" style="margin-bottom: 20px;">
        <img id="image" src="{{ asset(env('GLOBAL_PLUGIN_PATH').'/template/images/services/'.$services->data->image) }}" alt="{{ $services->data->image_ori }}">
      </div>
      <input type="file" name="file" class="form-control" id="uploadImage" accept="image/*">
    </div>
    <div class="col-md-12">
      <label for="formFile" class="form-label">Title</label>
      <input type="text" name="title" class="form-control" value="{{$services->data->title}}" required>
    </div>
    <div class="col-md-12">
    ...
    <div class="col-md-12">
      ...
      {!!$services->data->short_description!!}
      </textarea>
    </div>
    <div class="col-md-12">
    ...
    <div class="text-center">
      <input type="hidden" name="file_before" value="{{$services->data->image}}">
  ...
  $(document).ready(function() {
    ...
    $('#formInput').on("submit", function(event) {
      ...
      if ($('#formInput').valid()) { // Check if form is valid
        ...
        $.ajax({
          type: "POST",
          url: "{{ URL::asset(env('APP_URL').'/admin-page/services/do-add') }}",
          data: data,
          processData: false,
          contentType: false,
          dataType: "json",
