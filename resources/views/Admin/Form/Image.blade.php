  ...
  <form id="formInput" class="row g-3">
    <div class="col-md-7">
      ...
      <div class="image-container" style="margin-bottom: 20px;">
        <img loading="lazy" id="image" src="{{ asset(env('GLOBAL_PLUGIN_PATH').'/template/images/'.$data->data->image) }}" alt="{{ $data->data->image_ori }}">
      </div>
  ...
  $(document).ready(function() {
    ...
    $('#formInput').on("submit", function(event) {
      ...
      if ($('#formInput').valid()) { // Check if form is valid
        ...
        $.ajax({
          type: "POST",
          url: "{{ URL::asset(env('APP_URL').'/admin-page/home/do-add-staticImage') }}",
          data: data,
          processData: false,
          contentType: false,
          dataType: "json",
