                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="card-title">We Do</h5>
                            <button class="btn btn-success" dataaction="add" dataid="id" onclick="getaction(this)"><i class="fa fa-plus" aria-hidden="true"></i> Create We Do</button>
                        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        var dataTable = $('#data-table').DataTable({
            ...
            "ajax": {
                "url": "{{ URL::asset(env('APP_URL').'/admin-page/wedo/listdata') }}",
                "dataType": "json",
                "type": "POST",
                "data": {
                    "_token": "<?= csrf_token() ?>"
    ...
    function getaction(e) {
        ...
        if (action == 'delete') {
            swal({
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        $.get("{{ URL::asset(env('APP_URL').'/admin-page/wedo/do-delete') }}", {
                                id: arr[0]
                            },
                            function(data) {
                                swal.close();
        ...
        if (action == 'edit') {
            ...
            $.get("{{ URL::asset(env('APP_URL').'/admin-page/wedo/edit') }}", {
                    id: arr[0]
                },
                function(data) {
        ...
        if (action == 'add') {
            ...
            $.get("{{ URL::asset(env('APP_URL').'/admin-page/wedo/add') }}", {},
                function(data) {
                    $(".modal-content").html(data);
